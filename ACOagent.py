...
class ToMAgent:
    ...
    def choose_action(self, state, avail_action, t=0, evaluate=False):
        ...
        # 拼接输入：[state_encoding, character, mental]
        state_enc = torch.FloatTensor(self.env.encode_state(state)).unsqueeze(0)
        inputs = torch.cat([state_enc, self.e_character, self.e_mental], dim=-1)
        action_value = self.net.forward(inputs).squeeze()
        ...
        # 选好动作后，更新 mental embedding
        self.e_mental, self.hidden_state = self.observer.calc_mental(
            state, action, self.e_character, self.hidden_state
        )
    ...
    def learn(self):
        ...
        e_mentals = torch.stack(e_mentals, dim=0)  # e_t
        next_e_mentals = torch.stack(next_e_mentals, dim=0)  # e_{t+1}
        e_characters = self.e_character.expand_as(e_mentals)

        # calculate loss
        inputs = torch.cat([states, e_characters, e_mentals], dim=1)
        next_inputs = torch.cat([next_states, e_characters, next_e_mentals], dim=1)
