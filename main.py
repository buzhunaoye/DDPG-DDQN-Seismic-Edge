...
from modules.agents.PSOagent import GCNAgent
...
def main(name="tom", seed=0):
    ...
    if name == "local":
        ...
        path = r"D:\Pythonproject\DRLOFF(beifen) 1\saved\off\mlp\1766399149.305605\5000"
        if path != "":
            agent.load_models(path)
    elif name == "gcn":
            agent = GCNAgent(env)
            # TODO: 设置你训练好的 GCN 模型路径
            path = r"D:\Pythonproject\DRLOFF(beifen) 1\saved\off\gcn\1766407843.1655524\5000"
            if path != "":
                agent.load_models(path)
    elif name == "gat":
        agent = GATAgent(env)
        path = r"D:\Pythonproject\DRLOFF(beifen) 1\saved\off\gat\1766407519.7091844\5000"
        if path != "":
            agent.load_models(path)
    elif name == "tom":
        agent = ToMAgent(env)
        path = r"D:\Pythonproject\DRLOFF(beifen) 1\saved\off\tom\1766407940.246756\5000"
        if path != "":
            agent.load_models(path)
        agent.init_hidden()
...
if __name__ == "__main__":
    ...
    for name in ["local", "edge", "cloud", "random", "greedy", "mlp", "gcn", "gat", "tom"]:
        ...
        episodes = 100
        dvr_rate_mean = 0
        ep_reward_mean = 0
